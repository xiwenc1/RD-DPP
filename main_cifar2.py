# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 10:15:39 2023
...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 12 19:04:25 2023
...
path = f'exp_up_{args.T}_efficientnet/'
...
for run_index in range(args.repeat):
    ...
    model_eval_prox = copy.deepcopy(model_gt_temp).to(DEVICE)
    ...
    def pick_by_kcenter(eval_period):  #kcenter
            acc_accuracy_list = np.zeros((Total_budget,2,EPOCH))
            dataset_model_entropy_X= dataset_gt_X_temp
            dataset_model_entropy_Y = dataset_gt_Y_temp
            model_eval_prox = copy.deepcopy(model_gt_temp).to(DEVICE)
            pick_index1 = []
            ...
            feats_all = np.zeros((len(train_loader),num_class*num_feat))+1e-8
            y_all = np.zeros(len(train_loader))
            for num_trans in range(0,Total_budget,eval_period):
                # print(num_trans)
                ...
                for step,(x,y) in enumerate(train_loader):
                    ...
                    if (step not in pick_index1):
                        ...
                        # loss =  1/loss_func(model_eval_prox(dataset_entropy_X_temp.to(DEVICE))[0],dataset_entropy_Y_temp.type(torch.LongTensor).to(DEVICE)).item()
                        # loss =  loss_func(model_eval_prox(dataset_entropy_X_temp.to(DEVICE))[0],dataset_entropy_Y_temp.type(torch.LongTensor).to(DEVICE)).item()
                        # loss_gain_list[step] = loss
                ...
                SELECT_MODEL = kCenterGreedy(feats_all,y_all,seed=None)
                already_selected = copy.deepcopy(pick_index1)
                already_selected.extend(list(range(C_ini)))
                index = SELECT_MODEL.select_batch_(model=None,already_selected=already_selected,N= eval_period)
                # index = np.argsort(loss_gain_list)[0:eval_period]
                pick_index1.extend(index)
                ...
                if num_trans%eval_period ==0:
                    Dataloader_entropy = Data.DataLoader(TensorDataset(dataset_model_entropy_X,dataset_model_entropy_Y.type(torch.LongTensor)),batch_size=BATCH_SIZE,shuffle=True)
                    model_eval_prox = EfficientNetB0().to(DEVICE)
                    if multip_GPU:
                        model_eval_prox = torch.nn.DataParallel(model_eval_prox)
                    optimizer_entropy = torch.optim.SGD(model_eval_prox.parameters(),lr=args.LR,
                      momentum=0.9, weight_decay=5e-4)
                    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_entropy, T_max=200)
                    acc_train_e,acc_test_e = train_cifar(Dataloader_entropy,val_loader,DEVICE,EPOCH,model_eval_prox,optimizer_entropy,loss_func,scheduler)
                    # acc_train_e,acc_test_e =train_dis(Dataloader_entropy,val_loader,DEVICE,EPOCH,model_eval_prox,optimizer_entropy,loss_func,dataset_gt_X_temp,dataset_gt_Y_temp.type(torch.LongTensor),alpha = 0.1)
                    acc_accuracy_list[num_trans,0,:]=np.array(acc_train_e)
                    acc_accuracy_list[num_trans,1,:]=np.array(acc_test_e)
            Dataloader_entropy = Data.DataLoader(TensorDataset(dataset_model_entropy_X,dataset_model_entropy_Y.type(torch.LongTensor)),batch_size=BATCH_SIZE,shuffle=True)
            model_eval_prox = EfficientNetB0().to(DEVICE)
            if multip_GPU:
                        model_eval_prox = torch.nn.DataParallel(model_eval_prox)
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_entropy, T_max=200)
            acc_train_e,acc_test_e = train_cifar(Dataloader_entropy,val_loader,DEVICE,EPOCH,model_eval_prox,optimizer_entropy,loss_func,scheduler)
            return acc_accuracy_list,acc_test_e,pick_index1
    
    
    def pick_by_DPPsampling(eval_period):  #kcenter
            acc_accuracy_list = np.zeros((Total_budget,2,EPOCH))
            dataset_model_entropy_X= dataset_gt_X_temp
            dataset_model_entropy_Y = dataset_gt_Y_temp
            model_eval_prox = copy.deepcopy(model_gt_temp).to(DEVICE)
            ...
            for num_trans in range(0,Total_budget,eval_period):
                ...
                all_index = list(range(len(train_loader)))
                already_selected = copy.deepcopy(pick_index1)
                already_selected.extend(list(range(C_ini)))
                ava_index = list( set(all_index)-set(already_selected))
                
                
                DPP = FiniteDPP('likelihood', **{'L': feats_all[ava_index].dot(feats_all[ava_index].T)})
                
                
                # for _ in range(10):
                    
                DPP.sample_exact_k_dpp(size=eval_period, random_state=None)
                
                index_SAMPLE = DPP.list_of_samples
                index = np.array(ava_index)[index_SAMPLE]
                
                if isinstance(index[0], int) is False:
                    index = list(index[0])
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_entropy, T_max=200)
            acc_train_e,acc_test_e = train_cifar(Dataloader_entropy,val_loader,DEVICE,EPOCH,model_eval_prox,optimizer_entropy,loss_func,scheduler)
            assert(Total_budget==len(set(pick_index1)))
            return acc_accuracy_list,acc_test_e,pick_index1
    ...
    for eval_period in eval_period_list:
        ...
        print('DPP sampling Coreset')
        acc_dppsample_list,acc_dppsample_margin,dppsample_index = pick_by_DPPsampling(eval_period)
        
        
        print('k-Center Coreset')
        acc_kcener_list,acc_test_kcener,kcener_index = pick_by_kcenter(eval_period)
        ...
        acc_rand_list,acc_test_rand_all,rand_index = pick_by_random(eval_period)
    
    
    
#     result = [acc_kcener_list, acc_dppsample_list,kcener_index,dppsample_index]
#     results = np.array(result,dtype=object)
    result = np.zeros(7,dtype=object)
    
    result[0] = acc_dpp_list_old
    result[1] = acc_dppsample_list
    result[2] = acc_kcener_list
    result[3] = acc_entropy_list
    result[4] = acc_margin_list
    result[5] = acc_rand_list
    results = np.array(result,dtype=object)
    
    name =  f'{args.dataset_name}_additional_{run_index}'
